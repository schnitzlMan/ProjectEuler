    ...
    while currentVal != 1 and not sequenceKnown:
        ...
        if currentVal < keepListUpTo:
            if iterationsList[currentVal] != 0:
                sequenceKnown = True
                print('sequenceKnown for ', inVal, currentVal)
        #else:
    ...
    if lengthTempList > 0:
         for idx, val in enumerate(tempList):
            if val < keepListUpTo:
                ...
                if sequenceKnown == True: # Korrektur da iteration durch bekannte Lösung gestoppt wurde
                    iterationsList[val] += iterationsList[currentVal] -1
                # und jetzt ergänze auch alle *2 *2 *2 ... fache in der Liste nach oben
                iterVal = 2*val
                increaseBy = 1
                while iterVal < keepListUpTo:
                    iterationsList[iterVal] = iterationsList[val] + increaseBy
                    iterVal *= 2
                    increaseBy +=1
